...
export default function JumpClient({ id }: { id: string }) {
  const router = useRouter();
  const destination = getDestinationById(id);
  const [progress, setProgress] = useState(0);
  const [currentMilestoneIndex, setCurrentMilestoneIndex] = useState(0);
  const travelDuration = 8000; // ms
  ...
  useEffect(() => {
    ...
    const startAudio = async () => {
      ...
      synth = new Tone.MembraneSynth({ octaves: 4, pitchDecay: 0.1 }).connect(gain);
      noise = new Tone.Noise('pink').start();
      filter = new Tone.AutoFilter({ frequency: '2n', baseFrequency: 200, octaves: 4 }).toDestination().start();
      noise.connect(filter);
      lfo = new Tone.LFO('4n', 400, 4000);
      lfo.connect(filter.baseFrequency as unknown as Tone.ToneAudioNode);
      lfo.start();
    ...
    const interval = setInterval(() => {
      setProgress(prev => {
        const next = prev + (100 / (travelDuration / tick));
        if (next >= 100) {
          ...
          setTimeout(() => router.push(`/destination/${id}`), 1500);
          return 100;
        }
        audioContext.then(({ lfo, synth }) => {
          const pulseRate = 4 + (next / 100) * 12;
        ...
        if (nextMilestone && next / 100 >= nextMilestone.t) {
  ...
  }, [id, destination, currentMilestoneIndex, router]);
    ...
    <div className="flex flex-col items-center justify-center h-full w-full text-center">
      ...
      <div className="relative z-10 bg-background/50 backdrop-blur-md p-8 rounded-xl shadow-2xl border border-primary/20 max-w-4xl w-full">
          ...
          <div className="absolute top-1/2 -translate-y-1/2 w-full flex justify-between">
            {milestones.map((m, i) => (
              <div key={i} className="relative">
                <div className={cn('h-6 w-6 rounded-full bg-secondary border-2 flex items-center justify-center transition-all duration-300',
                  progress / 100 >= m.t ? 'border-accent bg-primary' : 'border-muted')}
                >
                  {progress / 100 >= m.t && <CheckCircle className="h-4 w-4 text-green-400" />}
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-5 gap-4 text-center">
          {milestones.map((m, i) => (
            <div key={i} className={cn('flex flex-col items-center gap-2 p-4 rounded-lg transition-all duration-300', i === currentMilestoneIndex ? 'bg-primary/30 scale-105' : 'opacity-50')}>
              <div className="w-16 h-16 bg-card/50 rounded-full flex items-center justify-center border border-transparent">{m.icon}</div>
              <span className={cn('font-semibold text-sm', i === currentMilestoneIndex && 'text-primary-foreground')}>{m.label}</span>
            </div>
          ))}
        </div>
      </div>
